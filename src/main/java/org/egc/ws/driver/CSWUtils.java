...
public class CSWUtils {
    static Logger logger = LoggerFactory.getLogger(CSWUtils.class);
...
//		String reqxml = grtreq.getValue().toString();
        ...
        for (int i = 0; i < rmts.size(); i++) {
            ...
            if (rmt.getConstraint().size() > 0) {
                ...
                logger.info(rmt.getConstraint().get(0).getAllowedValues().getValueOrRange().get(0).toString());
                if (rmt.getConstraint().get(0).getAllowedValues().getValueOrRange().get(0).equals("XML")) {
                    posturl = rmt.getHref();
                }
            }
        }
